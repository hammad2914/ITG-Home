...
const searchInput = document.querySelector("#test-modal-search");
...
jQuery(document).ready(function ($) {
  const trimStr = (str, max = 76) => {
    if (!str) return null;
    if (str.length > max) return `${str.substring(0, max - 3)}...`;
    return str;
  };

  const customPagination = (ctx, resultInfo) => {
    let maxPagesType,
      maxPages = -1,
      config = ctx.config;
    $.each(resultInfo, (documentType, typeInfo) => {
    ...
    let currentPage = resultInfo[maxPagesType].current_page,
      ...
      totalResults = resultInfo[maxPagesType].total_result_count;

    $(".test-search-footer").remove();
    $("#st-content-cache").remove();

    $(
      config.renderPaginationForType(
        maxPagesType,
        currentPage,
        totalPages,
        totalResults
  ...
  const customRenderResults = (ctx, data) => {
    let $resultContainer = ctx.resultContainer,
      config = ctx.config;

    $resultContainer.html("");

    $.each(data.records, (documentType, items) => {
      $.each(items, (_, item) => {
        ctx.registerResult(
    ...
    customPagination(ctx, data.info);
  };

  const customRenderFunction = (_, item) => {
    const html = `
    <div class="st-result">
      <a href="${item.url}" target="_blank">${
      trimStr(item.highlight.title) || trimStr(item.title)
    }
      <p style="padding: 1px; margin: 0px; color: #666">${trimStr(
        item.body,
        120
      )}</p></a>
    </div>
  ...
  const customRenderPagination = (_, currentPage, totalPages, totalResults) => {
    let html,
      paginationNumbers = "",
      paginationInfo,
      previousBtn,
    ...
    if (currentPage != 1) {
      ...
      previousBtn = `
        <a href="#" class="st-prev" data-hash="true" data-page="${previousPage}">
          &laquo; Previous
        </a>
      `;
    }
    ...
    if (currentPage < totalPages) {
      ...
      nextBtn = `
      <a href="#" class="st-next" data-hash="true" data-page="${nextPage}">
        Next &raquo;
      </a>`;
    }

    if (totalResults === 0) {
      paginationInfo = `<div>Showing no results for "${Swiftype.currentQuery}"`;
    } else {
      paginationInfo = `
        <div>Showing ${currentPage * 10 - 9}-${
        currentPage * 10 > totalResults ? totalResults : currentPage * 10
      } of ${totalResults} for "${Swiftype.currentQuery}"</div>
      `;
    }

    let paginationPage = currentPage === 1 ? currentPage++ : currentPage;
    paginationPage =
      currentPage + 3 < totalPages ? currentPage : totalPages - 3;

    if (totalResults !== 0 && totalPages > 5) {
      paginationNumbers = `
      <a href="#" class="st-pagination-btn ${
        paginationPage - 1 === currentPage && "active"
      }" data-hash="true" data-page="${paginationPage - 1}">
        ${paginationPage - 1}
      </a>
      <a href="#" class="st-pagination-btn ${
        paginationPage === currentPage && "active"
      }" data-hash="true" data-page="${paginationPage}">
        ${paginationPage}
      </a>
      <a href="#" class="st-pagination-btn ${
        paginationPage + 1 === currentPage && "active"
      ...
      <a href="#" class="st-pagination-btn ${
        paginationPage + 2 === currentPage && "active"
      }" data-hash="true" data-page="${paginationPage + 2}">
        ${paginationPage + 2}
      </a>
      <a href="#" class="st-pagination-btn ${
        paginationPage + 3 === currentPage && "active"
      }" data-hash="true" data-page="${paginationPage + 3}">
        ${paginationPage + 3}
      </a>
    `;
    }
  ...
  const customPreRenderFunction = (data) => {
    searchInput.value = Swiftype.currentQuery;
    return data;
  };

  $("#test-search-input").swiftypeSearch({
    renderFunction: customRenderFunction,
    resultContainingElement: "#test-results-container",
    engineKey: "PywFuCaYV1qHkWYtsVp7",
    renderPaginationForType: customRenderPagination,
    renderResultsFunction: customRenderResults,
    preRenderFunction: customPreRenderFunction,
  });

  const customAutocomplete = function (document_type, item, idx) {
    return '<p class="title">' + Swiftype.htmlEscape(item["title"]) + "</p>";
  };

  $("#test-search-input").swiftype({
    engineKey: "PywFuCaYV1qHkWYtsVp7",
    renderFunction: customAutocomplete,
    resultLimit: 5,
  });

  $("#test-modal-search").swiftypeSearch({
    renderFunction: customRenderFunction,
    resultContainingElement: "#test-results-container",
    engineKey: "PywFuCaYV1qHkWYtsVp7",
    renderPaginationForType: customRenderPagination,
  ...
  jQuery(window).on("load", () => {
    const results = document.querySelectorAll(".st-result");
    if (results.length !== 0) {
      searchModal.style.opacity = 1;
      searchModal.style.visibility = "visible";
    }
...
(function ($) {
  ...
  Swiftype.pingAutoSelection = function (engineKey, docId, value, callback) {
    ...
    var url =
      Swiftype.root_url + "/api/v1/public/analytics/pas?" + $.param(params);
    Swiftype.pingUrl(url, callback);
  };
  ...
  $.fn.swiftype = function (options) {
    ...
    return this.each(function () {
      ...
      var $swiftypeWidget = $(
        '<div class="' + config.widgetContainerClass + '" />'
      );
      ...
      var $list = $("<" + config.suggestionListType + " />").appendTo(
        $listContainer
      );
      ...
      $this.showNoResults = function () {
        ...
        if (config.noResultsMessage === undefined) {
          ...
          $list.append(
            $("<li />", { class: config.noResultsClass }).text(
  ...
  var processInput = function ($this) {
    ...
    if (
      typeof $this.data("swiftype-config-autocomplete").engineKey !==
      "undefined"
    ) {
  ...
  var defaultResultRenderFunction = function (ctx, results) {
    ...
    $.each(results, function (document_type, items) {
      $.each(items, function (idx, item) {
        ctx.registerResult(
          $(
            "<li>" + config.renderFunction(document_type, item, idx) + "</li>"
          ).appendTo($list),
          item
        );
      });
            ...
            * Javascript Triggers.
            ...
            $(document).once("key55").ready(function () {
                ...
                * Main Mega Menu JS
                ...
                $('.nav-item > a').once("key0").click(function (e) {

                    $(this).parent('li').toggleClass('responsive-height');
                    $('.nav-item > a').not($(this)).parent('li').removeClass('responsive-height');

                    $(this).addClass('item-active');
                    $(this).next('.submenu').toggleClass('mm-active');
                    $('.nav-item > a').not($(this)).removeClass('item-active');
                    $('.nav-item > a').not($(this)).next('.submenu').removeClass('mm-active');
                    $('.secondary-submenu').removeClass('secondary-submenu-active');
                    //submenu
                    $('.sub-nav-item-search > a').next('.secondary-submenu-full').removeClass('mm-active');
                ...
                $('.main-menu').once("key6").click(function (event) {
                    event.stopPropagation();
                });
                ...
                */
                ...
                $('.sub-nav-item-search > a').once("key1").click(function (e) {
                    $(this).next('.secondary-submenu-full').toggleClass('mm-active');
                    $('.nav-item > a').removeClass('item-active');
                    $('.nav-item > a').next('.submenu').removeClass('mm-active');
                    $('.secondary-submenu').removeClass('secondary-submenu-active');
                    e.preventDefault();
                });

                $('.sub-nav-item > a').once("key2").click(function (e) {
                    $(this).next('.secondary-submenu').toggleClass('secondary-submenu-active');
                    $('.sub-nav-item > a').not($(this)).next('.secondary-submenu').removeClass('secondary-submenu-active');
                    $('.nav-item > a').removeClass('item-active');
                    $('.nav-item > a').next('.submenu').removeClass('mm-active');
                    $('.secondary-submenu-full').removeClass('mm-active');
                    e.preventDefault();
                });

                $(window).once("key3").click(function () {
                        $('.submenu').removeClass('mm-active');
                        $('.nav-item > a').removeClass('item-active');
                        $('.sub-nav-item-search > a').next('.secondary-submenu-full').removeClass('mm-active');
                        $('.adds-components-widget-results').addClass('addsWg-hidden');
                        $('.secondary-submenu').removeClass('secondary-submenu-active');
                ...
                * Hamburger Menu
                ...
                $('.hamburger').once("key15").click(function(e) {

                    $(this).toggleClass('is-active');
                    $('.main-menu-list').toggleClass('main-menu-list-active');
                    $('#block-system-menu-block-blog-term-menu').toggleClass('blog-menu-show');
                    e.preventDefault();
                });
...
jQuery(function ($) {
  // adds higlighting of metdata and image alt text for french websites on CTRL + SHIFT + L key press or SHIFT + L in Mac
  /*document.addEventListener('keypress', function (e) {
    if(e.key === '\f' || e.key === 'l' || e.key === 'L') {
      if (window.location.pathname.indexOf('/fr/') !== -1 || window.location.href === 'http://itg.net.pk/fr') {
        var metaDesc = $('head').find('meta[name=description]').prop('content');
        $('body section.main-section').prepend('<p style="background-color: lightgreen; padding: 8px 16px; margin: 0px;"><i>META DESCRIPTION:</i> ' + metaDesc + '</p>');

        $('main img').each(function (i, e) {
          var alt = $(e).prop('alt');
          $(e).after('<p style="background-color: lightgreen; padding: 8px 16px; margin: 0px;"><small><i>ALT TEXT:</i> ' + alt + '</small></p>');
        });
      }
    }
  });*/
